...
function cloneResponse(response) {
    var copy = util_1.fastCloneDeep(response);
...
var Formio = /** @class */ (function () {
    ...
    Formio.prototype.index = function (type, query, opts) {
        var _url = type + "Url";
        query = query || '';
        if (query && util_1.isObject(query)) {
    ...
    Formio.prototype.uploadFile = function (storage, file, fileName, dir, progressCallback, url, options, fileKey, groupPermissions, groupId, uploadStartCallback, abortCallback) {
        ...
        var request = Formio.pluginWait('preRequest', requestArgs)
            ...
            return Formio.pluginGet('fileRequest', requestArgs)
                ...
                if (storage && util_1.isNil(result)) {
    ...
    Formio.prototype.downloadFile = function (file, options) {
        ...
        var request = Formio.pluginWait('preRequest', requestArgs)
            ...
            return Formio.pluginGet('fileRequest', requestArgs)
                .then(function (result) {
                if (file.storage && util_1.isNil(result)) {
    ...
    Formio.prototype.userPermissions = function (user, form, submission) {
        return Promise.all([
            ...
            if (form && form.submissionAccess) {
                for (var i = 0; i < form.submissionAccess.length; i++) {
                    ...
                    if (['create', 'read', 'update', 'delete'].includes(perm)) {
                        if (util_1.intersection(permission.roles, user.roles).length) {
            ...
            if (submission) {
                ...
                util_1.eachComponent(form.components, function (component, path) {
                    if (component && component.defaultPermission) {
                        var value = util_1.get(submission.data, path);
    ...
    Formio.makeRequest = function (formio, type, url, method, data, opts) {
        ...
        requestArgs.opts.headers = util_1.defaults(requestArgs.opts.headers, {
    ...
    Formio.request = function (url, method, data, header, opts) {
        ...
        var result = Formio.pluginAlter('wrapFetchRequestPromise', Formio.fetch(url, options), { url: url, method: method, data: data, opts: opts }).then(function (response) {
            ...
            return getResult.then(function (result) {
                ...
                if (range && util_1.isObject(result)) {
                    range = range.split('/');
                    if (range[0] !== '*') {
    ...
    Formio.pluginWait = function (pluginFn) {
        ...
        return Promise.all(Formio.plugins.map(function (plugin) {
            var _a;
            return (_a = (plugin[pluginFn] || Formio.noop)).call.apply(_a, __spreadArray([plugin], args));
        }));
    };
    Formio.pluginGet = function (pluginFn) {
        ...
        var callPlugin = function (index) {
            ...
            return Promise.resolve((_a = (plugin[pluginFn] || Formio.noop)).call.apply(_a, __spreadArray([plugin], args)))
                .then(function (result) {
                if (!util_1.isNil(result)) {
                    return result;
                }
    ...
    Formio.pluginAlter = function (pluginFn, value) {
        ...
        return Formio.plugins.reduce(function (value, plugin) {
            return (plugin[pluginFn] || Formio.identity).apply(void 0, __spreadArray([value], args));
        }, value);
    };
    /**
    ...
    Formio.requireLibrary = function (name, property, src, polling) {
        ...
        if (!Formio.libraries.hasOwnProperty(name)) {
            ...
            var plugin = util_1.get(window, property);
            if (plugin) {
