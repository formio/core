...
function mergeObject(src, dst) {
    lodash_1.each(src, function (value, key) {
...
function mergeArray(src, dst) {
    src.forEach(function (value) {
        var query = {};
        lodash_1.each(value, function (subValue, key) {
            if (!Array.isArray(subValue)) {
                query[key] = subValue;
            }
        });
        var dstValue = lodash_1.find(dst, query);
        if (dstValue) {
...
function rewind(submissions) {
    ...
    if (submissions && submissions.length) {
        submissions.forEach(function (sub) { return mergeObject(sub.data, submission.data); });
    }
    return submission;
}
...
function unwind(form, submission) {
    var dataPaths = {};
    var locked = {};
    var submissions = [lodash_1.fastCloneDeep(submission)];
    ...
    var setDataValue = function (dataPath, values, parent, offset, current) {
        ...
        while (lodash_1.has(locked, "[" + current + "]." + parent)) {
            if ((current + 1) >= submissions.length) {
                submissions.push(lodash_1.fastCloneDeep(submissions[current]));
            }
            current++;
        }
        ...
        if (!lodash_1.has(submissions[current].data, parentPath) &&
            submissions[current - 1] &&
            lodash_1.has(submissions[(current - 1)].data, parentPath)) {
            lodash_1.set(submissions[current].data, parentPath, lodash_1.fastCloneDeep(lodash_1.get(submissions[(current - 1)].data, parentPath)));
        }
        /* eslint-enable no-useless-escape */
        var pathValue = [];
        lodash_1.set(submissions[current].data, parent, pathValue);
        lodash_1.set(locked, "[" + current + "]." + parent, true);
    ...
    formUtil_1.eachComponent(form.components, function (component, path) {
        ...
        /* eslint-disable no-useless-escape */
        var paths = lodash_1.filter(path.replace(new RegExp(".?" + component.key + "$"), '').split('.'));
        /* eslint-enable no-useless-escape */
        if (!hasDataPath && paths.length) {
