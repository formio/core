...
function mergeObject(src, dst) {
    util_1.each(src, function (value, key) {
...
function mergeArray(src, dst) {
    src.forEach(function (value) {
        var query = {};
        util_1.each(value, function (subValue, key) {
            if (!Array.isArray(subValue)) {
                query[key] = subValue;
            }
        });
        var dstValue = util_1.find(dst, query);
        if (dstValue) {
...
function unwind(form, submission) {
    var dataPaths = {};
    var locked = {};
    var submissions = [util_1.fastCloneDeep(submission)];
    ...
    var setDataValue = function (dataPath, values, parent, offset, current) {
        ...
        while (util_1.has(locked, "[" + current + "]." + parent)) {
            if ((current + 1) >= submissions.length) {
                submissions.push(util_1.fastCloneDeep(submissions[current]));
            }
            current++;
        }
        ...
        if (!util_1.has(submissions[current].data, parentPath) &&
            submissions[current - 1] &&
            util_1.has(submissions[(current - 1)].data, parentPath)) {
            util_1.set(submissions[current].data, parentPath, util_1.fastCloneDeep(util_1.get(submissions[(current - 1)].data, parentPath)));
        }
        /* eslint-enable no-useless-escape */
        var pathValue = [];
        util_1.set(submissions[current].data, parent, pathValue);
        util_1.set(locked, "[" + current + "]." + parent, true);
    ...
    formUtil_1.eachComponent(form.components, function (component, path) {
        ...
        /* eslint-disable no-useless-escape */
        var paths = util_1.filter(path.replace(new RegExp(".?" + component.key + "$"), '').split('.'));
        /* eslint-enable no-useless-escape */
