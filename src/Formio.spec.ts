...
import { fastCloneDeep } from './util/util';
...
describe('Formio.js Tests', () => {
  ...
  describe('Plugins', () => {
    ...
    const testFileRequest = function testFileRequest(fnName: any, formUrl: any, args: any) {
      it(`Plugin ${fnName}`, (done) => {
        let step = 0;
        const testResult = { _id: 'TEST_ID', testResult: 'TEST_RESULT' };
        ...
        plugin.fileRequest = function(requestArgs: any) {
          assert.equal(++step, 4, 'request hook should be called fourth');
          assert.deepEqual(requestArgs, expectedArgs, 'Request hook arguments match expected arguments');
          return Promise.resolve()
            .then(() => {
        ...
        plugin.wrapFileRequestPromise = function(promise: any, requestArgs: any) {
        ...
        const formio: any = new Formio(formUrl);
