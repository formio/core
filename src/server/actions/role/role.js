...
const RoleAction = {
  ...
  async executor(scope) {
    return async (req, res, next) => {
      ...
      if (
        scope.action.settings.association !== 'existing' &&
      ...
      if (scope.action.settings.association === 'existing' && !scope.action.settings.role) {
        error(
          'Missing role for RoleAction association of "existing". Must specify role to assign in action settings or a form component named "role"',
        );
        return res
          .status(400)
          .send(
      ...
      if (scope.action.settings.association === 'existing' && !res.resource.item) {
        error(
          'Missing submission for RoleAction association of "existing:. Form must have a resource field named "submission".',
        );
        return res
          .status(400)
          .send(
